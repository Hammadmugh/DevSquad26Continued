...
export default function MessageBubble({ message }) {
  ...
  return (
    <div
      className={`flex flex-col ${isOwn ? "items-end" : "items-start"} mb-3 group`}
      onTouchStart={isOwn ? handleTouchStart : undefined}
      onTouchEnd={isOwn ? handleTouchEnd : undefined}
    >
      {!isOwn && (
      ...
      <div className={`flex items-end gap-1.5 ${isOwn ? "flex-row-reverse" : "flex-row"}`}>
        ...
        {isOwn && (
          <button
            ...
            className={`transition-opacity text-gray-400 hover:text-red-500 disabled:opacity-30 p-1 shrink-0 ${
